# 鱼骨图工作流设计器（YGT Flow）
> 当前版本：鱼骨图 V4.0（2026-09-11 定稿）。V1.0 功能基线及后续版本记录见 docs/ygt-version-history.md。

基于 AntV X6 3.1.8 的零构建画布编辑器，目标形态参照流行工作流设计器：左侧组件库拖拽、画布自由摆放连线、双击编辑文字、属性面板改样式、保存/导入导出。

## 当前进度

**鱼骨图 V4.0（2026-09-11 定稿）**

当前版本功能基线与后续版本记录统一维护于 [docs/ygt-version-history.md](docs/ygt-version-history.md)，README 不再逐条重复历史进度。

V4.0 定稿新增：新增“按层级重建布局”按钮，不读取 `positions` 位置，按上下级关系用旧 Raphael 算法重建并保存为 `positions`，下次默认按位置载入实现新旧兼容；同步实现 `legacyLine` 几何持久化与拖动挂点距离传播，拖动父线时子线挂点到父线起点的距离保持不变。V3.1 定稿新增：新增“按角度刷新鱼骨图”布局能力，按层级、order 奇偶和鱼头方向重算线段；无位置数据初始化改为奇偶布局并保持父子、上下、左右半平面，避免同级线段交叉；Oracle 模式启动时跳过自动建表。V3.0 定稿新增/收口：层级弹窗新增节点与连线按当前画布同层级样式继承，4/5 级未单独定义时沿用最近已定义级别的完整样式，通用鱼刺设置增加四级，子鱼刺新增/删除/排序后按兄弟顺序自动重算父线锚点比例；同步补充 Oracle 连接超时与独立打包配置、文档列表过滤刷新。V2.0 定稿新增：文档列表“新建鱼骨图”改为弹窗，包含名称输入、模板卡片与 7 套预设样式预览，创建后自动带入模板与样式进入编辑器。V1.2 新增：独立只读预览界面 `?docId=xxx&view=preview`，全屏仅保留缩放与 PNG/SVG/PDF 导出，支持按住空格 + 鼠标左键拖动平移（单击不拖动），触屏单指平移，不允许编辑。V1.1 新增：一二级鱼刺文字按框自动折行，超出显示省略号，拖动框体后即时重算；双击与属性面板支持回车换行。原有 V1.0 核心范围：7 套鱼形预设、模板、鱼头文字、四向端口与 edge-to-edge 连线、层级按线同步、悬空自动回连、层级树编辑、通用设置、导入导出、FastAPI 多库存储。
